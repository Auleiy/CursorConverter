...
BinaryWriter bw = new(new FileStream(outputPath, FileMode.OpenOrCreate, FileAccess.ReadWrite));
Bitmap bm = (Bitmap)Image.FromFile(inputPath);
...
#region 2 Bitmap information

// Bitmap scale
// 2 bytes
// width: byte, height: byte
// Set to 0 if greater than 255 (i don't know why)
uint width = (uint)bm.Width;
uint height = (uint)bm.Height;
...
Console.Write("Hotspot X: ");
ushort hotspotX = ushort.Parse(Console.ReadLine());
Console.Write("Hotspot Y: ");
...
{
    Console.WriteLine("Width or height should be divisible by 8");
    return;
}
